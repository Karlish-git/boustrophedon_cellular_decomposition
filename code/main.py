...
if __name__ == '__main__':
    ...
    bcd_out_im, bcd_out_cells, cell_numbers, cell_boundaries, non_neighboor_cell_numbers, cells = bcd.bcd(binary_map)
    ...
    exec_time_dfs = timeit.timeit('dfs.dfs(cleaned_DFS, graph4, starting_cell_number)',
                                  'from __main__ import dfs, cleaned_DFS, graph4, starting_cell_number',
                                  number=iter_number)
    ...
    exec_time_bfs = timeit.timeit('bfs.bfs(cleaned_BFS, graph4, starting_cell_number)',
                                  'from __main__ import bfs, cleaned_BFS, graph4, starting_cell_number',
                                  number=iter_number)
    ...
    x_coordinates = move_boustrophedon.calculate_x_coordinates(x_length, y_length, \
                                                               cell_numbers, cell_boundaries,
                                                               non_neighboor_cell_numbers)
    ...
    exec_time_hc = timeit.timeit('distance_optim.hill_climbing(optim_problem,starting_cell_number)', \
                                 'from __main__ import distance_optim, optim_problem, starting_cell_number',
                                 number=iter_number)
    ...
    path_time_mimic = timeit.timeit(
        'move_boustrophedon.track_paths(original_map,mimic_path,cell_boundaries,non_neighboor_cell_numbers)', \
        'from __main__ import move_boustrophedon, \
                                   mimic_path, original_map, cell_boundaries,non_neighboor_cell_numbers',
        number=iter_number)
